...
cmsyAlgorithm <- function(stock,res,start.yr,end.yr,r.low,r.hi,user.log.r,stb.low,stb.hi,int.yr,intb.low,intb.hi,endb.low,endb.hi,btype,force.cmsy,comment,duncert,sigR,yr,ct.raw,bt, q.start, q.end,species,name,region,subregion,group,source) {
  ...
  MCA <-  SchaeferMC(ri=ri1, ki=ki1, startbio=startbio, int.yr=int.yr, intbio=intbio, endbio=endbio, sigR=sigR,
                     pt=T, duncert=dataUncert, startbins=10, ni=ni, nyr=nyr, ct=ct)
  ...
  if (n.viable.b <= 1000){
    ...
    while (n.viable.b <= 1000 && current.attempts <= max.attempts){
      ...
      MCA <-  SchaeferMC(ri=ri1, ki=ki1, startbio=startbio, int.yr=int.yr, intbio=intbio, endbio=endbio, sigR=sigR,
                         pt=T, duncert=duncert, startbins=startbins, ni=2*ni, nyr=nyr, ct=ct)
      mdat.all <- rbind(mdat.all,MCA[[1]])
      rv.all   <- mdat.all[,1]
      kv.all   <- mdat.all[,2]
      btv.all  <- mdat.all[,3:(2+nyr+1)]
  ...
  if(length(rv.all[rv.all > 0.9*start.r[2]]) < 5) {
    ...
    MCA <-  SchaeferMC(ri=ri1, ki=ki1, startbio=startbio, int.yr=int.yr, intbio=intbio, endbio=endbio, sigR=sigR,
                       pt=T, duncert=duncert, startbins=10, ni=ni, nyr=nyr, ct=ct)
    mdat.all <- rbind(mdat.all,MCA[[1]])
    rv.all   <- mdat.all[,1]
    kv.all   <- mdat.all[,2]
    btv.all  <- mdat.all[,3:(2+nyr+1)]
  ...
  if(write.output == TRUE) {
    ...
    output = data.frame(as.character(group),
                        as.character(region),
